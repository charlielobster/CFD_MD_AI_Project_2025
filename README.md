...
Some of these examples in this repository are about how to use Graph Neural Networks (GNNs), Physics-Informed Neural Networks (PINNs), and Fourier Neural Operators (FNOs) Convolutional Neural Networks (CNNs), or related software components to solve second order PDEs. Use these to augment results from standard CFD and MD algorithms, the traditional discrete PDE solvers for Navier-Stokes or LBM, etc. There are good reasons to do this because, for example, the output of a FNO-based AI model is continuous and mesh-invariant, and therefore scalable to arbitrary resolution and times.
